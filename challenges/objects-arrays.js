...
*/
function makeDinoObject(name, diet, weight, length, period) {
  const dino = {
    name: name,
    diet: diet,
    weight: weight,
...
const contactInfo = graduates.map(uni => uni.first_name + " " + uni.email);
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */

const unisWithUni = graduates.splice(uni => uni.university += "Uni");
...
zooAnimals.forEach(arrItem => {
  displayNames.push(
    `Name: ${arrItem.animal_name}, Scientific: ${arrItem.scientific_name}`
  );
});
...
zooAnimals.map(arrItem => {
  lowCaseAnimalNames.push(arrItem.animal_name.toLocaleLowerCase());
});
...
*/
const populationTotal = zooAnimals.reduce((total, arrItem) => {
  return (total += arrItem.population);
}, 0);
